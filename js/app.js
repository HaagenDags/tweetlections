	...
	function displayData(data, emotion) {
		...
		getUserInfo(data, function(user){
			...
			if(document.querySelector('.'+user.state)) {
				tally[user.state] = (tally[user.state] || {democratic: 0, republican: 0});
				tally[user.state][emotion.type] = (tally[user.state][emotion.type] || 0) + 1;

				var stateEl = document.querySelector('.'+user.state);
				stateEl.style.fill = (tally[user.state].democratic > tally[user.state].republican) ? democraticColor : ((tally[user.state].democratic < tally[user.state].republican) ? republicanColor :neutralColor); 

				stateEl.setAttribute('data-democratic', tally[user.state].democratic);
